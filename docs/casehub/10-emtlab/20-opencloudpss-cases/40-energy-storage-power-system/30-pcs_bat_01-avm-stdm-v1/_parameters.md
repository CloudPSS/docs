...
| 参数名 | 键名 | 类型 [单位] | 描述 |
|:------ |:---- |:-----------:|:---- |
| 储能电站额定容量 | `Sbase` | 实数 [MVA] | 储能电站额定容量 |
| 有功功率指令 | `P_cmd` | 实数 [MW] | 通过潮流计算结果写回 |
| 无功功率指令 | `Q_cmd` | 实数 [MVar] | 通过潮流计算结果写回 |
...
| 参数名 | 键名 | 类型 [单位] | 描述 |
|:------ |:---- |:-----------:|:---- |
| 储能电站名称 | `BAT_Name` | 文本 | 储能电站名称（用于输出电压穿越相关事件日志） |
| 输出有功功率（MW） | `P_BAT` | 虚拟引脚（输出） | 输出有功功率（MW） |
| 输出无功功率（MVar） | `Q_BAT` | 虚拟引脚（输出） | 输出无功功率（MVar） |
| 电压穿越状态 | `State_BAT` | 虚拟引脚（输出） | 电压穿越状态电压穿越运行状态（±3:高低电压穿越失败；±2:高低电压穿越阶段； ±1:高低电压恢复阶段；0:正常运行） |
