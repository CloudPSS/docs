...
$$
 \eta_q + \eta_p+\eta_l = 1 
 $$  
 式中：F 是燃料提供的总功率(kW),P 是发电机的额定功率(kW),$\eta_p$代表发电效率,$\eta_l$代表燃料发电机组的损失系数。
...
| 采购成本 | `MaxPowerGen` | 万元/台 | 采购成本 | 实数 | 设备采购成本 |
| 固定运维成本 | `MinPowerGen` | 万元/年 | 固定运维成本 | 实数 | 设备固定运维成本 |
| 可变运维成本 | `MaxPowerGen` | 元/kWh | 可变运维成本 | 实数 | 设备可变运维成本 |
...
| 引脚名 | 键值 (key)  | 维度 | 描述 |
| :--- | :--: | :--- | :--- |
| 电接口 | `DC/AC` | 1×1 | 可以在引脚处输入相同的字符使得燃料发电机组与其他电元件相连，当基础参数**元件类型**项是**直流元件**时，键值为**DC**；**元件类型**项是**交流元件**时，键值为**AC**。|
