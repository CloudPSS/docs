...
水力方程
 ...
 式中：${\Delta}p$是进出口压差(kPa)，$p_{in} 、p_{out}$分别为流体进出口压力(kPa)，$k$是局部压降系数 (kPa/(m³·s⁻¹)²), $m$是质量流量(kg/s)，$ρ$是密度（kg/ m³），$Q_{heat}$是热源一次侧的供热功率(kW)，$\eta$是制冷机的冷热比，$Q_{cool}$是冷源二次侧的出口制冷功率(kW)，$h_{in},h_{out}$分别为工质的进出口比焓（kj/kg）。
...
:   3. **运行优化**
        ...
        此时吸收式制冷机元件的出力由系统优化计算得到，但是**不读取规划参数**，需要将**优化参数**的**是否优化该设备**项设置为**是**即可。
