...
水力模型：  
 ...
 $$  
 式中：${\Delta}p$是进出口压差(kPa)，$p_{in} 、p_{out}$分别为流体进出口压力(kPa)，$k$是局部压降系数 (kPa/(m³·s⁻¹)²), $m$是质量流量(kg/s)，$ρ$是密度（kg/ m³），$P$是额定用电功率(kW)，$\eta$是供热效率，$h_{in},h_{out}$分别为工质的进出口比焓（kj/kg）。
...
:   3. **运行优化**
        ...
        此时燃气锅炉元件的出力由系统优化计算得到，但是**不读取规划参数**，需要将**优化参数**的**是否优化该设备**项设置为**是**即可。
