...
综合能源系统建模仿真平台主要面向**区域电能、热能、太阳能、风能、电池与储能**等多种能源相耦合的综合能源系统，通过建模仿真技术分析系统运行特性。平台由**数据管理**、**拓扑建模**、**仿真模拟**和**运行优化**模块构成，**数据管理模块**可以对多种类型的基础数据进行录入和管理，**拓扑建模**模块用于系统拓扑结构的搭建以及边界条件设置，**仿真模拟**模块用于仿真参数设置和计算结果的展示，**运行优化**模块用于优化参数设置和计算结果的展示。
