...
本文档说明如何在不同智能体（Agent）中接入 **CloudPSS MCP** 服务器，以及如何完成身份认证。
...
根据当前所处环境，选择对应的 MCP 接入地址：

| 环境 | MCP 接入地址 |
| --- | --- |
| **测试网络 - 局域网** | `http://cloudpss-calculate.local.ddns.cloudpss.net/api/mcp` |
| **测试网络 - 内网** | `http://cloudpss-calculate.ddns.cloudpss.net/api/mcp` |
| **正式发布环境(暂未发布)** | `https://cloudpss.net/api/mcp` |

:::tip
下文中所有示例均使用正式发布环境地址 `https://cloudpss.net/api/mcp`，请根据实际环境替换为对应地址。
:::


### 身份认证

**CloudPSS MCP** 服务器支持通过身份认证流程完成登录授权。如果当前智能体不支持该认证流程，可使用 [SDK Token](../../software/50-user-center/40-general-account-settings/30-sdk-token-managemment/index.md) 方式进行认证。
...
2. 如需使用测试环境地址，请将 `url` 替换为对应地址，例如：

```bash
# 测试网络 - 局域网
claude mcp add-json cloudpss-mcp '{ "type": "http", "url": "http://cloudpss-calculate.local.ddns.cloudpss.net/api/mcp" }'

# 测试网络 - 内网
claude mcp add-json cloudpss-mcp '{ "type": "http", "url": "http://cloudpss-calculate.ddns.cloudpss.net/api/mcp" }'
```
...
```json
{
  "mcp": {
    "cloudpss-mcp": {
      "type": "remote",
      "url": "http://cloudpss-calculate.local.ddns.cloudpss.net/api/mcp",
      "enabled": true
    }
  }
}
