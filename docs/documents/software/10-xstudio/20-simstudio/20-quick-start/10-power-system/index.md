...
本节以**RLC 电路的电磁暂态仿真**和**五节点电力网络的潮流计算仿真**为例，介绍使用 SimStudio 平台进行电力系统项目建模仿真的方法。
...
- 此外，电气元件本身支持部分内部量输出，无需外加量测元件。如在电阻元件中添加量测，点击电阻元件,在参数的 `Monitoring` 中填写以 `#` 号开头的量测信号输出名称。更多关于信号名称的信息详见 [参数变量及引脚体系](../../50-modeling/10-params-variables-pins/index.md)。
...
示例模型下载连接： [五节点电力网络的潮流计算仿真](./case-2.cmdl)