...
<table>
    ...
    <tr>
        <td>宽度</td>
        <td>控制该组件的宽度。一般填写5的倍数，详见[规范](./70-simstudio-icon-guideline/index.md)。宽度的设置不影响```路径```和```引脚```类型组件的实际显示样式。当填写负数时，相当于水平翻转该组件。</td>
    </tr>
    <tr>
        <td>高度</td>
        <td>控制该组件的高度。一般填写5的倍数，详见[规范](./70-simstudio-icon-guideline/index.md)。高度的设置不影响```直线```、```路径```和```引脚```类型组件的实际显示样式。当填写负数时，相当于垂直翻转该组件。</td>
    </tr>
    <tr>
        <td rowspan="6">**排列**</td>
        <td>隐藏</td>
