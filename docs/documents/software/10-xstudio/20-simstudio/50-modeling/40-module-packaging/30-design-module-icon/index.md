...
在[元件/模块封装](../index.md)的过程中，打开 [接口标签页](../../../40-workbench/20-function-zone/20-interface-tab/index.md)，右侧为图标绘制工作区，包含```图标组件库```、```图标绘制区```、```显示设置与属性编辑区```，如下图所示。
...
本节详细介绍各个图标组件的基础属性，阅读本节后，用户将能自行绘制任意需要的元件/模块图标。
...
<table>
    ...
    <tr>
        <td>填充颜色</td>
        <td>点击选色按钮![选色图标](image-15.png)可以弹出调色板，用于选择填充颜色和透明度。对```矩形```、```直线```、```椭圆```，默认为白色；对```路径```默认为透明；对```文本```默认为黑色；对```引脚```、```直线```无效。</td>
    </tr>
    <tr>
        <td>数据</td>
        <td>条件</td>
...
- 部分CloudPSS旧版本的元件图标和当前的新版本图标不兼容，利用这些元件另存得到的元件中，图标绘制工作区中会出现```导入当前图形```的按钮。
...
- 条件模块
    ...
    在绘图区中，存在组件的**条件**设置为```mode==1```时，```窗格```的**条件**栏会显示```mode==1```的条目，如下图所示。
