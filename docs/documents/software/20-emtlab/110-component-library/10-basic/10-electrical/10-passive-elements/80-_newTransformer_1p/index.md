...
对于两绕组理想变压器，设匝比为 $a$，并以绕组 1 为参考侧，将绕组之间的等效漏感定义为从绕组 1 端口测得的漏感 $L$（$L=L_1+a^2L_2$）。在这种建模形式下，绕组电流导数与端口电压之间可直接表示为：
...
对于两绕组变压器，$R_1$、$R_2$ 用于模拟铜损耗；$R_{1nll}$、$R_{2nll}$ 用于模拟空载损耗；$I_s(t)$为注入电流源，用于模拟变压器的饱和特性。
