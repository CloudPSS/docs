...
理想电压源输出三相对称正弦波形。若采用正序相序，各相内部电动势瞬时值可表示为：
...
**电阻电抗直接输入**：直接输入各序电阻和电抗：

$$
Z = R + jX
$$

**阻抗幅值和相角输入**：输入阻抗幅值 $|Z|$ 和阻抗角 $\varphi$，换算为：
...
$$
\mathbf{V_{port,0}} = \mathbf{E_{0}} - \mathbf{Z_{eq}} \cdot \mathbf{I_{port,0}}
$$

其中，$\mathbf{E}_0$ 为内部电动势初始值，$\mathbf{V}_{port,0}$ 为端口初始电压，$\mathbf{I}_{port,0}$ 为由初始有功、无功功率和端口电压计算得到的端口初始电流，由此可以得到三相戴维南等值电压源的内部电动势。
