...
RLC 矩阵不仅可用于普通多支路 R、L、C 网络，也可用于等效建模具有多端口耦合关系的电气设备。

左侧案例中，RLC 矩阵用于等效 π 型传输线。通过配置多支路电阻、电感和电容矩阵，可同时描述线路的串联阻抗、对地电容以及相间耦合关系，从而替代由多个离散 R、L、C 元件搭建的 π 型线路模型。

右侧案例中，RLC 矩阵用于等效变压器。通过将原边、励磁支路和副边绕组表示为多条矩阵支路，并在电感矩阵中设置自感和互感参数，可描述绕组间磁耦合关系。该方式适合用于需要自定义绕组参数、耦合关系或特殊接线结构的变压器等效建模场景。

[下载 RLC 矩阵进阶仿真案例](./RLC_comptest.cmdl)
