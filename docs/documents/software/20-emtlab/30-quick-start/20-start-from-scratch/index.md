...
:::warning
1. 最好提供一个单机无穷大系统算例（包含潮流求解和电磁暂态仿真）。5节点过于复杂了，可以保留，作为第三个案例（这个算例考察重点是如何批量修改参数）。(7月底前完成)
2. 注意按照规范要求调整该文档。(优先级低)
:::


本文档介绍三个入门案例：**RLC 电路的电磁暂态仿真**、**单机无穷大系统的潮流计算和电磁暂态仿真**和**五节点电力网络的潮流计算仿真**。用户可跟随帮助文档步骤，搭建入门案例熟悉 EMTLab 的图形化建模、电磁暂态仿真和潮流计算功能。
...
<TabItem value="case2" label="单机无穷大系统的潮流计算和电磁暂态仿真">
...
- 同步发电机： `Rated Voltage (L-G, RMS)` 设置为 10.5 / sqrt(3)。
- 三相交流母线：
  - 三相交流母线 1 和 三相交流母线 2 的 `Base Voltage (L-L, RMS)` 设置为 230。
  - 三相交流母线 3 和 三相交流母线 4 的 `Base Voltage (L-L, RMS)` 设置为 110。
- 三相双绕组变压器:
...
#### 添加测量元件
- 在左侧**模型**栏中点击**量测**分别选择**电流表**和**电压表**元件拖拽至工作空间，在三相双绕组变压器 1 与 三相交流母线 1 之间添加**电流表**（需要删除原有连线，重新连接元件和电流表）。在三相双绕组变压器 2 与三相交流母线 4 之间添加**电压表**，可直接将**电压表**与原有连线相连。分别点击**电流表**和**电压表**出现参数配置面板，选择 `Dimension` 为 Three Phrase，设置 `Name for Current Signal [kA]` 为 I，设置 `Name for Voltage Signal [kV]` 为 V。
...
- 点击**启动任务**开始潮流计算仿真，在结果页面由 `Buses` 和 `Branches` 两个潮流数据表格显示潮流计算结果。

![潮流计算结果 =x350](./power-flow-result-1.png)

- 点击**潮流回写数据**选项框下的**修改项目文件**按钮，弹出的确认框中点击**应用**，将潮流结果回写至模型中。
...
- 点击**启动任务**开始电磁暂态仿真，在结果页面可以查看到仿真结果如下图所示：
...
<TabItem value="case3" label="五节点电力网络的潮流计算仿真">
