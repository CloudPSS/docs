...
本文档介绍了 3 部分内容。第一部分为信号量测，介绍了信号的种类、信号的量测方法和输出通道元件。 第二部分为配置示波器输出通道，介绍输出通道配置方法，包括示波器分组名称、采样频率、窗口类型、窗口宽度、示波器输出通道的概念介绍和配置方法。第三部分为仿真结果编辑，包括对示波器窗口的各类操作、结果保存等操作。
