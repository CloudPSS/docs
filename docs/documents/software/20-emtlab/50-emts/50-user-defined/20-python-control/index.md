...
![直接创建 Python 元件](directly-create.png "直接创建 Python 元件")
...
此时，在**实现标签页**中将出现拓扑、电磁暂态等子标签页。若当前算例是拓扑实现的算例，可右键拓扑子标签页点击删除。再选中**电磁暂态**子标签页，点击**创建电磁暂态 - Python 实现**按钮，即可创建 Python 用户自定义控制元件。

![删除拓扑实现](topology-implementation.png "删除拓扑实现")

![创建电磁暂态 - Python 实现](create-python.png "创建电磁暂态 - Python 实现")

若当前算例是其它电磁暂态实现的算例，可右键电磁暂态子标签页，将当前电磁暂态实现删除，再点击**创建电磁暂态 - Python 实现**按钮，创建一个新的 Python 实现。
...
:::info
若一个元件中同时存在拓扑实现和 Python 实现，将以 Python 实现优先，拓扑实现将被忽略。
:::
...
- **外部触发**：
    ...
    在元件参数面板的 Trigger 栏填入控制信号，Python 元件将按该信号的上升沿触发被调用。
...
至此，一个使用 Python 编写的分段函数元件构建完成。保存的元件可以使用工作台右上角的搜索功能查询。若同时设置了**元件标签**，则该元件还可在**拓扑实现页面-元件列表**中对应的元件标签分组中显示。

![元件分组](./case-component-tag.png "元件标签与元件分组")
    
- 控制效果验证

    新建一个普通项目，在右上角搜索栏中，搜索**元件RID**（本案例中：`用户名/Python_Test1`），定位到新建的 Octave 分段函数元件，将其拖入画布。
...
至此，一个带有全局变量并使用固定频率触发的 Python 元件构建完成。保存的元件可以使用工作台右上角的搜索功能查询。若同时设置了**元件标签**，则该元件还可在**拓扑实现页面-元件列表**中对应的元件标签分组中显示。
...
- 控制效果验证
    ...
    新建一个普通项目，在右上角搜索栏中，搜索**元件RID**（本案例中：`用户名/Python_Test2`），定位到新建的 Octave 阶梯爬坡信号发生器元件，将其拖入画布。在其参数面板设置触发频率为 1 Hz，初始值为 10，爬升高度为 20。在其输出引脚连接一个**输出通道**观测输出结果。
...
至此，一个含多维输入、多维输出并使用外部触发的 Python 元件构建完成。保存的元件可以使用工作台右上角的搜索功能查询。若同时设置了**元件标签**，则该元件还可在**拓扑实现页面-元件列表**中对应的元件标签分组中显示。
...
- 控制效果验证
    ...
    新建一个普通项目，在右上角搜索栏中，搜索**元件RID**（本案例中：`用户名/Python_Test3`），定位到新建的 Octave 三相瞬时功率计算元件，将其拖入画布。
...
:
    需要注意的是，在存在**电磁暂态**标签页的具体实现时，**拓扑**的实现方法将会失效。用户可以右键选择电磁暂态的子标签，并点击**删除**按钮，删除 Python 的元件实现方法，保留拓扑实现方法，如下图所示。当然，也可以根据需要，自行选择需要保留的实现方法类型。
    
    ![删除一个实现方法 =x300](./delete-python-implementation.png "实现方法冲突时，需删除一个实现方法")

为什么带有 Python 元件的算例运行很慢？

:   
    对于含有 Python 元件的算例，在仿真的过程中需要根据 Python 元件的触发频率调用 Python 的解释器，导致仿真速度较慢。用户可以根据需求，选用拓扑模块封装等速度更快的方式进行模块构建。
