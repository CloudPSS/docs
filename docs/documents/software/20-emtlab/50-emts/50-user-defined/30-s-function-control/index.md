...
+ 编译来源 1： MATLAB/Simulink 子系统编译生成 S-Function 时自动生成的代码文件。  

+ 编译来源 2：用户自定义函数和接口的代码文件。  
...
用户需要获取 Simulink 子系统或自定义代码文件的**全局参数**、**输入/输出接口**信息，并根据信息定义 S-Function 元件的参数和引脚，在平台导入 `.so` 文件，完成自定义 S-Function 元件的构建。
...
- a. 解算器设置
    ...
    CloudPSS 中的系统均为离散系统，为使仿真结果一致，S-Function 也应使用离散解算器。

    :::warning
    若 S-Function 子系统中存在难以离散化的连续状态，不得不选用连续积分解算器时，可在 CloudPSS 的全局参数中定义 S-Function 实现的连续积分方式，详见 [常用全局参数：连续积分方式](./index.md#连续积分方式)。
    
    目前适配的连续积分方式有 ode1、ode2、ode3、ode4、ode5、ode8 和 ode1be，不同的积分方式会影响仿真结果准确度和仿真计算时间。该积分方式只对 S-Function 实现元件有效，CloudPSS 系统的求解依然是离散的。因此，该选择会使 CloudPSS 仿真结果与 Simulink 仿真结果出现一定误差。
    :::
...
- d. 设置 S-Function 参数
    ...
    :::info
    MATLAB R2022a 及以上版本，在生成 S-Function 时不支持全局参数可调。若有需要调整的参数，建议在子系统封装时改为引脚输入；或降低 MATLAB 版本，建议使用 MATLAB R2016b 至 R2020b。
    :::
...
- a. 新建空白 S-Function 实现元件  
    ...
    选择**总览标签页**，将**模型类型**切换为**元件**。
    
    ![切换为元件类型](./1-8-2.png "切换为元件类型")  

    此时，在**实现标签页**中将出现拓扑、电磁暂态等子标签页。选中**电磁暂态**子标签页，点击**创建电磁暂态 - S-Function 实现**按钮，即可创建自定义 S-Function 元件。

    ![创建电磁暂态 - S-Function 实现](./1-8-3.png "创建电磁暂态 - S-Function 实现")  

    :::info
        若一个元件中同时存在拓扑实现和 S-Function 实现，将以 S-Function 实现优先，拓扑实现将被忽略。可右键拓扑子标签页，点击删除拓扑实现。
    :::


- b. 参数引脚列表定义  

    选择**接口标签页**，在参数列表添加参数，在引脚列表添加引脚，并绘制元件图形。参数键名必须与 S-Function 引用的全局参数名一致；引脚键名不作要求，但顺序必须分别与 S-Function 的输入输出引脚对应。  

    ![添加元件引脚并绘制图形](./1-8.png "添加元件引脚并绘制图形")   

- c. 导入 .so 文件  

    在**实现标签页**导入步骤 2 生成的 `.so` 文件，并填写**入口函数名**。  
...
- d. 元件保存  
    ...
    至此，一个 S-Function 元件构建完成。保存的元件可以使用工作台右上角的搜索功能查询。若同时设置了**元件标签**，则该元件还可在**拓扑实现页面-元件列表**中对应的元件标签分组中显示。
...
### 常用全局参数
    ...
    在**接口标签页**中，除了添加 S-Function 本身的全局参数，还可以手动添加一些内置的全局参数，实现高级功能的配置。
    :::info
    内置全局参数以两个下划线开头。为避免系统识别错误，用户自定义的其它全局参数名只能以字母开头，不能以数字或下划线开头。
    :::  

#### 采样倍率

    S-Function 实现的采样步长固定为生成 S-Function 时子系统的采样步长。在某些应用场景中，子系统的采样步长可能与仿真步长不一致。Simulink 中，要求子系统采样步长为仿真步长的**整数倍**，如仿真步长为 **10 us**，而子系统以 **50 us**（仿真步长的 **5** 倍）进行采样与计算。  

    CloudPSS 中内置了**采样倍率**的全局参数来应对这种情况。用户可根据 S-Function 的采样需求选择是否添加这个参数，当不添加这个参数时，其值默认为 **1**，即 S-Function 采样步长与仿真步长一致。  

    该参数键名为：**__SFUNC_STEP**，添加方法如下：

    ![添加采样倍率参数](./4-1.png "添加采样倍率参数")   

#### 连续积分方式
   
    针对 Simulink 中不同阶数的连续积分解算器，CloudPSS 进行了适配，支持的连续积分方式有 ode1、ode2、ode3、ode4、ode5、ode8 和 ode1be。  

    ![支持的 Simulink 连续积分解算器](./4-2.png "支持的 Simulink 连续积分解算器")   

    在 CloudPSS 中，内置了定义**解算器类型**的全局参数，参数键名为：**__SFUNC_SOLVER**，参数值为与目标解算器类型同名的**字符串**。例如，可将该参数定义为**选择型**参数，其键值分别为 ode1、ode2、ode3、ode4、ode5、 ode8 或 ode1be。  
    ...
    :::warning
        不同的积分方式会影响仿真结果准确度和仿真计算时间。该积分方式只对 S-Function 实现元件有效，CloudPSS 系统的求解依然是离散的。因此，该选择会使 CloudPSS 仿真结果与 Simulink 仿真结果出现一定误差。
    :::

#### 数值规范化
    
    在浮点数运算中，当发生无效操作（如将 0 作为除数、对负数平方根等）时，编译器会按照 IEEE 754 标准生成 NaN 或 Inf 结果，而这些特殊值在后续计算中可能引发异常。

    为避免该问题，CloudPSS 中内置了**数值规范化**的全局参数，参数键名为：**__SFUNC_NORMALIZE**，参数值为 0 或 1。
    
    + 当参数值为 0 时，不进行数值规范化处理； 
    + 当参数值为 1 时，将计算中的输入、输出以及状态的 NaN 和 Inf 强制转换为有效的浮点数：将 NaN 转换为 0，将 ±Inf 转换为 ±1e308。
   
    当不添加这个参数时，其值默认为 1。

    ![添加数值规范化参数](./4-6.png "添加数值规范化参数")   

#### 库元件名称

    当一个算例中存在多个相同的 S-Function 实现元件，或需要多次调用相同的 .so 文件时，可设置**库元件名称**的全局参数，使库元件名称相同的 .so 文件只加载一次，避免重复加载导致算例文件过大，也可避免多次调用影响仿真运行速度。
    
    参数键名为：**__SFUNC_LIB_NAME**，文本类型。对于算例中该参数相同的 S-Function 实现元件，其 .so 文件只加载一次。

    :::info
    库元件名称的文本内容可以不是 .so 的文件名称，也可填入其它自定义名称。
    :::

    ![添加库元件名称参数](./4-7.png "添加库元件名称参数")   
...
- a. 自定义曲线元件导入 .csv   
    ...
    使用 **CloudPSS 控制-非线性函数**库中的自定义曲线元件，在元件参数面板的**编辑数据**中直接导入刚才生成的 `.csv` 文件。  

    ![csv 数据导入](./2-4.png "csv 数据导入")   

- b. 数据输入  

    自定义曲线的输入是**控制-基础**库中的时间输入元件，输出连到 S-Function 元件的对应引脚。  
...
- d. 构建自定义 S-Function 元件  
    ...
    切换到**实现标签页**，选中**电磁暂态**子标签页，点击**创建电磁暂态 - S-Function 实现**按钮。

    ![创建电磁暂态 - S-Function 实现](./1-8-3.png "创建电磁暂态 - S-Function 实现")  

    在**接口标签页**的参数列表定义栏，新建参数组，并添加参数，参数键名与 S-Function 的可调参数名一致，参数类型设为**实数**，输入类型为**常量**，默认值可与 Simulink 模型初始化页面的赋值一致。  
    ...
    在**实现标签页**，导入 `.so` 文件，填写**入口函数名**。注意，入口函数名不是 `.so` 文件的名称，而是 S-Function 的名称，本案例中是 **Control_sf**。  
...
## 常见问题
如何设置 S-Function 的采样步长？

:   
    S-Function 实现的采样步长固定为生成 S-Function 时子系统的采样步长，默认与仿真步长一致。若要修改 S-Function 的采样步长，可添加全局参数**__SFUNC_STEP**，详见 [常用全局参数：采样倍率](./index.md#采样倍率)。


S-Function 生成时不得不使用连续积分求解器，如何在 CloudPSS 中选择 S-Function 的连续积分方式？

:   
    CloudPSS 中的系统均为离散系统。若 Simulink 中不得不选择连续积分求解器，可添加全局参数**__SFUNC_SOLVER**，选择适配不同阶数的连续积分方式，详见 [常用全局参数：连续积分方式](./index.md#连续积分方式)。
