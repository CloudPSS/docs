...
- **添加虚拟输出端口**
    ...
    在风机并网模型中，添加 6 个虚拟输出端口元件，分别连接风机并网模型的三相并网电流、直流侧电压、以及风机的有功功率和无功功率，用于观测多机实时仿真的结果。虚拟输出端口元件的使用方法可详见软件工具 [CloudPSS Signal Monitor](../../../../software-tools/20-signal-monitor/index.md)。

     ![添加虚拟输出端口元件](./12-1.png "添加虚拟输出端口元件")
...
- **实时仿真方案设置**
    ...
    在风机并网模型的实时仿真设置中，编辑曲线型输出通道，设置需要观测的实时仿真结果。
    
    ![曲线型输出通道编辑](./13.png "曲线型输出通道编辑")
