...
本文档主要介绍电磁暂态实时仿真的原理和应用场景。为满足电磁暂态实时仿真对平台计算性能的要求，该功能需搭配硬件产品 [CloudPSS MiniRT](../../../hardware/10-desktop-type/10-cloudpss-mini/index.md) 或 [CloudPSS ProRT](../../../hardware/20-rack-type/10-cloudpss-pro/index.md) 使用，公网平台暂不支持电磁暂态实时仿真。
...
:::info
    ...
    **数据输入**指的是在每个仿真步长开始时，系统从外部设备或仿真环境中获取的信息，主要可包括：
      + 外部控制信号：如断路器状态、继电器动作信号、控制器设定值等；
      + 测量信号：来自物理装置或模拟传感器的电压、电流、功率等；
      + 外部事件指令：如故障投入/切除、负荷投切、新能源出力变化等。
  
    这些数据通常通过I/O接口（如网口、串口、光口、模/数转换接口等）输入仿真系统。
    
    **数据输出**指的是在每个仿真步长的系统求解完成后，系统将仿真结果输出至外部接口，主要可包括：
      + 仿真电气量结果：电压、电流、功率计算值；
      + 设备状态信息：如开关状态、保护动作情况；
      + 控制反馈信号：如PWM调制波、控制器输出指令等。
  
    输出数据主要用于驱动物理装置、实时显示波形、与外部设备交互等，通常通过I/O接口（如网口、串口、光口、数/模转换接口等）输出。
:::
...
考虑到仿真器与其它设备的数据交互通信耗时，目前支持基于 CPU 的最小实时仿真积分步长为 **10 us**。
...
- **b. 输出通道**
    ...
    更多关于实时仿真的波形观测、指令下发、录波等功能，详见 [实时信号监视工具](../../../software-tools/20-signal-monitor/index.md) 帮助文档。
