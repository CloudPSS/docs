...
- **b. 输出通道**
    ...
    - 曲线型输出通道：仿真结果将以曲线的形式显示，可设置**分组名称**、**窗口类型**、**窗口宽度**，并选择待观测的**输出通道**。窗口类型可选择为**移动窗口**或**示波器窗口**，详见 [电磁暂态仿真计算-量测和输出系统-输出窗口类型](../50-emts/30-meters-and-outputs/index.md#输出窗口类型)。同一个分组中可选择多个输出通道。可添加多个分组。
