...
如果需要详细配置各处理器核心放置的分区，请参考[负载均衡功能](#负载均衡功能)部分。
...
### 自动聚合母线
自动聚合母线功能可将电网中的悬空母线进行聚合，可以进一步提高仿真效率。

### 负载均衡功能

负载均衡功能是为优化并行仿真效率而设计的关键功能，其核心作用是通过合理分配电网拓扑中的元件分区与计算逻辑核心的对应关系，平衡各核心的计算负载，从而提升整体仿真计算的速度和稳定性。

#### 负载均衡参数配置

负载均衡配置功能可在分网拓扑分析的同时，生成```负载均衡配置策略表```。只需在**基本设置**中打开```生成负载均衡策略表```开关即可，其余参数与前述分网功能参数配置一致。

![基本设置](image-12.png)

配置好参数后，点击```启动任务```按钮或按快捷键 ```Ctrl+R```，即可开始交直流电网拓扑分析计算。
计算完成后，在结果标签页会依次展示三张表：

1. 电气节点统计表——按分区列出元件类型及数量，并给出对应的预估耗时（单相节点数）；
2. 控制节点统计表——按分区给出对应的预估耗时（控制节点数）；
3. 负载均衡表——展示每个计算逻辑核心所分配的元件分区。

![电气节点统计表](image-13.png)

![控制节点统计表](image-14.png)

![负载均衡表](image-16.png)

在**结果标签页**的**负载均衡信息**卡片中，点击**修改项目文件**按钮，即可将生成的负载均衡策略表写回模型。

![写入负载均衡信息](image-17.png)

此外，用户还可在**结果标签页**的**修改项目文件**卡片中，点击**修改项目文件**按钮，使拓扑以色块形式显示分网结果。

![在拓扑中显示分网结果](image-18.png)

此时，在拓扑中将鼠标悬停到任意元件上，就能查看该元件对应的分区编号。需要注意的是，若元件为传输线，会同时显示其两端的分区编号。

![查看分区编号](image-19.png)

若用户需要手动配置负载均衡策略，则需在**高级设置**中打开```配置负载均衡策略```开关。此时，点击```编辑数据```按钮即可手动编辑负载均衡策略表。

![高级设置](image-20.png)

![手动编辑负载均衡策略表](image-21.png)
