...
式中：${\Delta}p$是进出口压差（$\mathrm{kPa}$），$p_{in} 、p_{out}$分别为流体进出口压力（$\mathrm{kPa}$），$k$是局部压降系数（$\mathrm{kPa/(m^3 \cdot s^{-1})^2}$）, $m$是质量流量（$\mathrm{kg/s}$），$ρ$是密度（$\mathrm{kg/m^3}$），$Q_{heat}$是热源一次侧的供热功率（$\mathrm{kW}$），$\eta$是制冷机的冷热比，$Q_{cool}$是冷源二次侧的出口制冷功率（$\mathrm{kW}$），$h_{in},h_{out}$分别为工质的进出口比焓（$\mathrm{kJ/kg}$）。
