...
额定转速下：
$$
H_{r} = a_{p}Q_{p}^{2} + b_{p}Q_{p} + c_{p}
$$
根据相似定律推导变工况特性：
$$
...
| 额定转速 | `RatedPumpSpeed` | rpm | 额定转速 | 实数 | 额定转速 |
| 最大工作转速 | `MaxPumpSpeed` | rpm | 最大工作转速 | 实数 | 最大工作转速 |
| 最小工作转速 | `MinPumpSpeed` | rpm | 最小工作转速 | 实数 | 最小工作转速 |
| 功率因数 | `PowerFactor` |  | 功率因数 | 实数 | 功率因数，范围 0-1，一般在 0.85-1 之间 |
| 额定转速下特性曲线 | `PerformanceCurve` |  | 额定转速下特性曲线 | 表格 | 输入额定转速下特性曲线，不同**流量**（`MassFlowrate`）下的**扬程**（`Lift`）和**效率**（`Effiency`） |
