...
设备信息库中将所有电冷热氢综合能源设备分为发电设备、制冷制热设备、制氢设备、储能设备、配电系统传输设备和氢能系统传输设备6类：

| 大类设备         | 子类设备                                                                           |
| :--------------- | :--------------------------------------------------------------------------------- |
| 发电设备         | 风机、光伏、燃料发电机组、燃料电池（质子交换膜燃料电池和固体氧化物燃料电池）       |
| 制冷制热设备     | 燃气锅炉、热泵、热管式太阳能集热器、单工况电压缩制冷机、制冷制冰双工况电压缩制冷机 |
| 氢源设备         | 电解水制氢槽（碱性电解槽、质子交换膜电解槽和固体氧化物电解槽）                     |
| 储能设备         | 蓄电池、储热罐、储冷罐、储氢罐、蓄冰槽、储氨罐                                     |
| 配电系统传输设备 | 变压器、模块化多电平换流器MMC、传输线                                              |
| 氢能系统传输设备 | 氢气压缩机                                                                         |

### 设备管理

打开设备信息库并选择特定子类设备后，进入该类设备的管理页面。

### 新建与删除

点击**新建一行**按钮，会自动创建一个新的可选设备。

在页面上方的**搜索输入框**，在输入框中输入设备名称等任意关键字，即可进行模糊搜索或筛选（关键字不区分大小写）。
...
能否修改平台模型？
:   IESLab平台暂不支持修改元件设备模型。

能否自定义元件模型？
:   IESLab平台自定义元件模型的功能即将发布，敬请期待。

能否修改典型库？
:   CloudPSS IESLab 官网的典型库不支持修改，对于服务器版本，管理员账户可以修改典型库，修改后该服务器下所有用户即可导入。

设备经济性参数有什么用？
:   设备经济性参数主要用来计算项目经济指标，采购成本用来计算项目投资，固定运维成本按年计费，可变运维成本按用量，用来计算项目运营费用。
