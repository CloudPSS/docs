...
>
> **延伸阅读（与《优化计算》模块衔接）**：本文档讲"拓扑层如何把参数设对"；更底层的**优化模型原理**（目标 / 变量 / 约束 / 惩罚项）与**结果正确性判定方法**见 [《优化计算·优化数学模型》](../../../60-optimization-module/10-fundamentals/index.md#4-优化数学模型) 与 [《优化计算·结果正确性判定》](../../../60-optimization-module/10-fundamentals/index.md#9-结果正确性判定)；求解失败后的逐类排查见 [《优化计算·结果查看·优化调试技巧》](../../../60-optimization-module/30-results-tab/index.md#优化调试技巧)。
...
下图以风机为例展示"容量配置范围"的设定界面——容量上下限共同决定优化变量的可行域（来自 [《优化计算·设备选型与定容》](../../../60-optimization-module/10-fundamentals/index.md#31-设备选型与定容)）：

![风机容量配置范围示例](./cap_limit.jpg "风机容量配置范围示例")

> **选型边界的本质**：容量上限/下限共同决定优化变量的可行域——下限即"至少装多少"、上限即"最多装多少"；上限过小会人为压制经济性最优配置，下限大于 0 表示"本项目必须上该设备"。更系统的约束分类（系统级合规约束 vs 设备运行约束）见 [《优化计算·约束条件》](../../../60-optimization-module/10-fundamentals/index.md#44-约束条件)。
...
- **惩罚项量级**：缺电惩罚（电不平衡惩罚 ）、弃电惩罚 等决定"违反约束的代价"，量级设错会直接改变优化取向。其原理与默认取值见 [《优化计算·惩罚项》](../../../60-optimization-module/10-fundamentals/index.md#45-惩罚项)。
...
> 更完整的"结果正确性判定清单"见 [《优化计算·判定清单 可打印对照》](../../../60-optimization-module/10-fundamentals/index.md#99-判定清单-可打印对照)，可与本文档自检清单配合使用。
