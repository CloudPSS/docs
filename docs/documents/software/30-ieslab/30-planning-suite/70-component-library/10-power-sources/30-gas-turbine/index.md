...
常见的燃气轮机能够支持热电联供，即CHP（Combined Heating and Power），CHP是指以天然气为主要燃料带动燃料发电机组、微燃机或内燃机发电机等燃气发电设备运行，产生的电力供应用户的电力需求，系统发电后排出的余热通过余热回收利用设备(余热热水锅炉等)向用户提供热水。通过这种方式大大提高整个系统的一次能源利用率，实现了能源的梯级利用。还可以提供并网电力作能源互补，整个系统的经济收益及效率均相应增加。


平台燃气轮机模型为**边际成本**模型。在经济学和金融学中，边际成本指的是每新增一单位生产的产品（或者购买的产品）带来的总成本的增量。那么，燃气轮机的边际成本是每增加1单位发电量所需的额外成本，包含**燃料成本和运维成本**。其数学模型如下：


$$ 
C_M=\frac{\partial (C_{fuel}+C_{om})}{\partial P} 
$$  

 式中：$C_M$ 是边际成本, $C_{fuel},C_{om}$ ,分别是燃料成本和运维成本，P是燃料发电机组的出力功率。
 燃料成本占比最大，边际成本主要取决于燃料耗率的微增率

燃料功率模型应用较为广泛，与平台边际成本模型关系密切:
$$ F=aP^2+bP+c$$
其中,a,b,c为机组特性常数。

燃气轮机的实际运行状态（如负荷率、启动状态等）会显著影响燃料耗率。例如，在低负荷下，燃气轮机的燃料耗率通常高于额定负荷下的耗率，这是因为低负荷运行时燃烧效率较低。
对功率求导，得到边际成本模型：
$$ \dot{F} =2aP+b $$ 


:::tip
+ 平台中的燃料发电机组机组**无法实现热电联供**，请注意！
+ 一般a为一个相对小的正数，边际成本为单调递增；因此平台限制燃料机组的边际成本曲线为非递减；
+ 对于常见的平均成本，其典型特征为反曲线，机组煤耗随功率增大逐渐降低；需要用户自行转换为边际成本曲线。
:::
...
| 最小发电容量配置 | `MiniPowerGenCapacity` | kW | 设备的最小发电容量配置 | 实数 | 仅当**待选设备类型**选择数据管理模块输入的设备后生效。|
...
元件模型是否准确？
:   燃料发电机组组模型较为简单，平台主要关注燃料与电之间的转换关系，并未关注发电、压缩、燃烧、透平等电热过程。



燃料发电机组可以使用煤炭为燃料吗？
:   可以的，IESLab 平台的燃料发电机组为燃料转换为电的通用模型，不仅可以使用天然气，还可以使用煤炭、甲烷等做燃料。
...
:   在对燃料发电机组元件进行规划设计前，务必录入编辑元件的**基础参数**和**规划参数**；
