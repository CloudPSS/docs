...
| 光伏板倾角 | `DipAngle` | ° | 配置光伏板倾角 | 实数（常量） | 仅当**出力模式**项为**由气象数据计算**且**光伏板跟踪方式**项为**固定倾角**时生效。调节范围 0-360° |
| 最小装机容量配置 | `MiniPanelCapacity` | kW | 设备的最小装机容量配置 | 实数 | 仅当**待选设备类型**选择数据管理模块输入的设备后生效。 |
| 最大装机容量配置 | `MaxPanelCapacity` | kW | 设备的最大装机容量配置 | 实数 | 仅当**待选设备类型**选择数据管理模块输入的设备后生效。 |
<!--
...
:   根据经验，光伏的最佳倾角在维度附近，而平台计算板面辐照 G = DHI + DirHI/sinα・sin (α+β)，忽略了太阳方位角，安装方位角，赤纬角等，而用天顶角计算时在早晚的低高度角时，计算误差很大，由于辐照在低高度角时计算偏大，拉高权重，最终让最佳倾角偏大。
...
:   光伏系统的**规划参数**的**出力模式**项配置为**指定出力曲线**即可，替换注意事项： 1. 配置容量上下限需一致，不能为零，不支持对配置容量进行优化，以配置确定的设备数量； 2. 配置范围上下限一致时，上传总出力曲线；3. 配置范围上下限不一致时，支持上传单台设备出力特性曲线，注意非标幺化（如非 1MWp 装机的标幺化出力曲线，二是一台光伏设备的出力曲线）