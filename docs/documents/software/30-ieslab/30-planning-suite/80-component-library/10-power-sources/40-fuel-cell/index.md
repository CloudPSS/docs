...
| 额定能耗 | `PowerGenEff` | $\mathrm{kWh/Nm^3}$ | 额定能耗 | 实数 | 单位燃料（1 Nm<sup>3</sup>氢气）的发电量（kWh） |
| 额定热电比 | `CHPEff` |  | 额定热电比 | 实数 | 燃料电池组的热电联产效率系数，为热与电功率之比，一般在0.4-0.8之间 |
...
| 最小发电容量配置 | `MiniElectricCapacity` | $\mathrm{kW}$ | 设备的最小发电容量配置 | 实数 | 仅当**待选设备类型**选择数据管理模块输入的设备后生效。|
| 最大发电容量配置 | `MaxElectricCapacity` | $\mathrm{kW}$ | 设备的最大发电容量配置 | 实数 | 仅当**待选设备类型**选择数据管理模块输入的设备后生效。|
