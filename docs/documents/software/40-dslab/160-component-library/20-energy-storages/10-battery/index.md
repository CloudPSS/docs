...
| 储能设备绑定 | `DeviceSelection` |  | 储能设备绑定 | 选择 | 选择数据管理模块中创建的储能设备模型 |
| 储能设备配置台数 | `DeviceNumber` | 台 | 储能设备配置台数 | 正数 | 储能设备配置台数 |
| 储能运行策略绑定 | `PowerCurve` |  | 储能运行策略绑定 | 选择 | 选择数据管理模块中创建的储能运行策略模型 |
| 初始S0C | `initialSoC` | % | 初始S0C | 实数 | 初始S0C， 0-100 |
