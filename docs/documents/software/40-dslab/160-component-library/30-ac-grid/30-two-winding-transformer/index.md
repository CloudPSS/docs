...
| 绕组#1额定电压有效值 | `V1` | kV | 绕组#1额定电压有效值 | 实数 | 绕组1额定电压有效值（所填变压器参数的电压基值） |
| 绕组#2额定电压有效值 | `V2` | kV | 绕组#2额定电压有效值 | 实数 | 绕组2额定电压有效值（所填变压器参数的电压基值） |
...
| 引脚名 | 键值 (key)  | 维度 | 描述 |
| :--- | :--: | :--- | :--- |
| Pin #1 | `0` | 3×1 | 一次侧接口，可以在引脚处输入相同的字符使得元件与其他元件相连|
| Pin #2 | `1` | 3×1 | 二次侧接口，可以在引脚处输入相同的字符使得元件与其他元件相连|
