...
本文档介绍 **个人中心** - **AppStudio 云空间** 的主要功能，包括打开空白项目、通过模板创建、上传文件等功能。 
...
成功登录后，在 **CloudPSS 个人中心** 点击 **AppStudio** 的图标后，页面将跳转至 **AppStudio 云空间**。如图所示，该界面包含快速开始、项目中心两部分区域。

![AppStudio 云空间](appstudio-cloud-space.png "AppStudio 云空间")


### 快速开始

**AppStudio 云空间** 提供了多种快速开始 AppStudio 项目的方法。

+ **打开空白项目**：点击**打开空白项目**，进入 **AppStudio 工作台** 并自动打开 **欢迎使用向导**。用户可进一步基于向导新建/打开/快速入门 **AppStudio** 项目。
+ **通过模板创建**：点击**通过模板创建**，进入 **AppStudio** 模板页面选择模板，点击进入**AppStudio 工作台**。
+ **上传文件**：点击**上传文件**，选择待上传的项目文件，文件支持 **zip** 和 **capp** 格式，详情参看[项目导入](#项目导入)。
...
**AppStudio 云空间** 的项目管理区提供了项目打开、收藏、查看历史版本、查看文档、删除、导出等功能，方便用户高效管理大量应用项目。

### 项目基本操作

鼠标移至任一个项目文件，会显示**打开发布链接**、**复制 rid**、**打开** 和 **更多选项** 操作按钮。
...
在个人项目列表中选中某项目，点击 **更多选项** - **收藏** 按钮，会弹出**收藏**、**收藏至新的分组**多个功能按钮。
...
项目列表中选中某项目，点击 **更多选项 - 查看历史版本** 按钮，弹出该项目的历史版本记录。点击任意一条历史版本记录可以进入 **AppStudio 工作台**。
...
**AppStudio** 项目中心支持以下方法导入项目应用。

+ **上传应用压缩包**：用于上传导出到本地的应用压缩包( **zip** 格式)。
+ **上传应用文件**：用于上传并打开保存在本地的项目文件( **capp** 格式)。

#### 上传应用压缩包

1. 点击**上传文件**按钮，选择待上传的应用压缩包

2. 页面弹出如下界面可选：**上传至用户**或**上传至组织**

3. 如果该压缩包在导出时设置了密码，需要在上传文件后输入该密码进行验证。

4. 密码验证成功，弹出**选择需要上传的模型**界面，允许用户自行选择需要上传的应用模型，点击**确定**。

5. 如果密码验证失败，弹出错误提示并取消本次上传服务。

![上传应用压缩包](upload-zip.png "上传应用压缩包")

#### 上传并打开

1. 点击**上传文件**按钮，选择待上传的应用文件。

2. 跳转到 **AppStudio 工作台**并直接打开该应用文件。

3. 如果该应用文件存在密码，需要在上传文件后输入该密码进行验证。

![密码保护](upload-file.png "密码保护")

### 工具栏

**AppStudio** 项目中心工具栏，包含以下三类工具按钮。
