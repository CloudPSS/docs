...
:::info
使用 Python 语言构建自定义电磁暂态仿真元件。  
:::
本节以一个可调斜率和分段点的分段函数，展示 Python 元件的创建和使用方法。

## 新建 Python 元件  

在CloudPSS SimStudio工作台，点击左上角的`新建`，在弹出的窗口中，选择项目模板下的**空白 Python 元件**。点击窗口右下角的新建按钮，即可完成新建。

![新建 Python 元件](./1.png "新建 Python 元件")
...
至此，一个使用 Python 语言的分段函数元件构建完成。可以新建一个`普通项目`，在对应的`元件标签`中找到该元件，并验证仿真结果。

![Python 元件结果验证](./13.png "Python 元件结果验证")

![Python 元件效果展示](./output.png "Python 元件效果展示")
