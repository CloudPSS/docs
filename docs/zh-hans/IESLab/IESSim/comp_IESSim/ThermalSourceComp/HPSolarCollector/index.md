...
>**在仿真参数出输入设备的仿真边界条件**
> 
> **启停策略**：选择`由气象数据计算`时，可以录入各个时间段对应的设备启停策略，注意不要超过设备配置台数，若仿真时间缺少响应的运行参数，平台采用插值和外推算法填充，且由于热管式太阳能集热器仅有启停策略（定功率模式），无法作为供热系统的平衡节点，需同其他供热设备一起供能。

### 优化参数

>**在运行优化前，需要设置优化参数**
> 
> **是否优化该设备**：选择“是”，则设备的运行策略由系统优化得到，系统不读取仿真参数的运行策略；选择“否”，则系统按照仿真参数的启停策略运行