...
**注意**：所有绕组的首端都是同极性端，当选择所有绕组都为Y连接时，变压器的联结组为Yy0(1#2#)及Yy0(1#3#)。
...
设已知参数为短路损耗$\Delta P_{12}$，$\Delta P_{23}$，$\Delta P_{31}$(单位：KW)，短路试验电压百分比$V_{12}\%$，$V_{23}\%$，$V_{31}\%$(单位：p.u.)，具体解算公式如下$^{[2]}$:
...
[1]刘天琪. 现代电力系统分析理论与方法[M]. 中国电力出版社, 2007.
