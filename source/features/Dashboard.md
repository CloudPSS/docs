...
CloudPSS的Dashboard功能提供了仿真数据可视化功能，借助图形化手段，用户可更加清晰有效的查看或传递仿真结果信息。对任意含有`输出通道`的仿真文件，都可使用Dashboard功能对仿真数据进行动态显示。打开任意仿真，点击`电磁暂态->仿真控制->打开Dashboard`，将弹出新的页面，如下图所示：
...
点击`下一步`将弹出配置表格对话框，该对话框与所选图表类型相关。

图表类型选择为曲线图时，配置表格对话框如下图所示，横坐标类型可选取为线性或对数，点击添加按钮，首先填写曲线名称，其次填写x轴的数据来源，可以选择为Default(时间)为X轴，也可选择为其他(该下拉框的选项来自于仿真工作空间的“输出通道”)。再次，选择Y轴的数据来源(该下拉框的选项来自于仿真工作空间的“输出通道”)。点击右侧编辑按钮可进一步设置曲线的样式特性，如图标、大小、线宽、颜色等等，如图所示。
...
图表类型选择为柱状图时，配置表格对话框如下图所示。

![柱状图配置](Dashboard/D3.png "柱状图类型的表格配置")

图表类型选择为饼图时，配置表格对话框如下图所示。
