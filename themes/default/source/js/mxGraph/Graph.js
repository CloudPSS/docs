...
Graph = function(container, model, renderHint, stylesheet, themes) {
    ...
    this.cellRenderer.getLabelValue = function(state) {
        ...
        if (state.view.graph.isHtmlLabel(state.cell)) {
            if (state.style['html'] != 1) {
                result = mxUtils.htmlEntities(result, true);
            } else {
    ...
    if (typeof mxVertexHandler !== 'undefined') {
        ...
        mxConstants.HIGHLIGHT_OPACITY = 100
        var marker = new mxCellMarker(this, '#ff0000');
        this.addMouseListener({
            mouseDown: function() {},
...
Graph.prototype.convertValueToString = function(cell) {
    ...
    //20161223 chart
    if (cell.thutype == 'channelChart') {
        mxClient.NO_FO = true;
        var node = document.getElementById('channel' + cell.id);
        if (!node || cell.chartTypeChange) {


            if ($('#channel' + cell.id).highcharts()) {
                $('#channel' + cell.id).highcharts().destroy();
            }

            cell.chartTypeChange = 0;
            node = document.createElement('div');
            node.id = 'channel' + cell.id;
            node.style.width = cell.geometry.width + 'px';
            node.style.height = cell.geometry.height + 'px';
            document.body.appendChild(node);

            if (charLayer[cell.chartType]) {
                charLayer[cell.chartType](node, cell);
            }
        } else {
            if ($('#' + node.id).highcharts()) {
                node.style.width = cell.geometry.width + 'px';
                node.style.height = cell.geometry.height + 'px';
                $('#' + node.id).highcharts().reflow();
            }
 ...
 * 添加直接选择父层函数
 * @Author   zdm
 * @DateTime 2016-11-02T17:51:32+0800
...
mxStencilRegistry.getStencil = function(name) {
    var result = mxStencilRegistry.stencils[name.toLowerCase()];
...
if (typeof mxVertexHandler != 'undefined') {
    (function() {
        ...
        Graph.prototype.click = function(me) {
            ...
            if (me.state && me.state.cell.thutype == 'oscChart') {
                setMonitor(me.state.cell.id, _pssSystem[me.state.cell.id]['chart']);
            } else {
                if (_pssSystem['0']) {
                    setSysMonitor('0', _pssSystem['0']['chart']);
                }
            }
...
function configureStylesheet(graph) {
    ...
    style[mxConstants.STYLE_GRADIENTCOLOR] = '#C1C1C1';
    style[mxConstants.STYLE_FILLCOLOR] = '#C1C1C1';
    style[mxConstants.STYLE_SWIMLANE_FILLCOLOR] = '#ffffff';
    style[mxConstants.STYLE_STROKECOLOR] = '#C1C1C1';
